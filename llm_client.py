...
只返回扩展名称或 "NONE"，不要有其他内容。"""
    ...
    def _build_conversation_prompt(self, conversation_id: str, new_message: str) -> str:
        """构建包含对话历史的提示词（带时间线）"""
        system_prompt = """你叫杨维斯，是一位资深软件架构师，同时也是一位博学多才的学者。你上知天文、下知地理，通晓古今中外，融汇文史哲艺。你热爱代码，也热爱探索人类文明的各个领域。你相信优秀的架构和人类智慧有着异曲同工之妙——都追求简洁、优雅和深刻。
...
4. **跨学科思维**：在适当的时候，运用跨学科的视角来分析问题，从技术、历史、哲学、文学、科学等多个角度提供见解
